...
# BUAN 370 Data Mining Project
# Dataset: Supply chain transactions with 1000+ records
# ============================================================================
# Structure:
...
cat("Missing values:", sum(is.na(supply_chain_df)), "\n")
cat("Duplicate orders:", sum(duplicated(supply_chain_df$Order_ID)), "\n")
...
#TIDY DATA - Show structure of cleaned dataset
str(supply_chain_df)
# ============================================================================
# 3. DATA QUALITY: INITIAL EXPLORATION
# ============================================================================
...
ggplot(supply_chain_df, aes(Supply_Risk_Flag)) +
  geom_histogram() +
  labs(title = "Supply Risk Flag Distribution")

ggplot(supply_chain_df, aes(Supplier_Reliability_Score)) +
  geom_histogram() +
  labs(title = "Supplier Reliability Score Distribution")
...
plot_9 <- ggplot(monthly_trends_mode, aes(x = order_month, y = avg_delay, 
...
supplier_performance <- supplier_performance %>%
  mutate(reliability_group = cut(Avg_Reliability, 
                                 breaks = 5,
                                 labels = c("Lowest: .5-.6", "Low: .6-.7", "Medium: .7-.8", 
                                            "High: .8-.9", "Highest: .9-1.0")))
...
relationship_risk <- sqldf("
  SELECT 
    Buyer_ID, 
    Supplier_ID, 
    (MAX(Historical_Disruption_Count) * 1.0 / AVG(Available_Historical_Records)) AS Raw_Rate,
    MIN(1.0, MAX(Historical_Disruption_Count) * 1.0 / MAX(Available_Historical_Records)) AS Plot_Rate
  FROM supply_chain_df
  GROUP BY Buyer_ID, Supplier_ID
  HAVING Plot_Rate > 0.2
  ORDER BY Raw_Rate DESC
")
